...
class GestionLocationVoiture extends AbstractController
{
    ...
    public function listReserved_user(LocationVoitureRepository $repository, Security $security)
    {
        $user = $security->getUser();

        $locationVoitures = $repository->createQueryBuilder('lv')
        ->where('lv.user = :user')
        ->andWhere('lv.status = :status')
    ...
    public function reserveVoiture_user(Request $request, $id, LocationVoitureRepository $repository, Security $security, ManagerRegistry $managerRegistry)
    {
        $user = $security->getUser();
